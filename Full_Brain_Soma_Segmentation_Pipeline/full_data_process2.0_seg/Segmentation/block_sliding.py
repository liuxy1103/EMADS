...
parser.add_argument("-c", "--ckp_file",type=str,default="/gdata/liuxy/unet_weight_map_loss/ckps_tri_gaussin_n/checkpoint-epoch91.pth")
...
flag3, num_d = cal_crop(z, z_stride, z_patch)
...
#save_path = os.path.join(save_path,'output.nii.gz')
save_path = os.path.join(save_path,block_path.split("/")[-1][:-10]+'outputgaussin_n.nii.gz')
block =  save_nii(init_block,save_path)
print("save save_path successfully:",time.time()-t2)
