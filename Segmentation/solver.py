...
class Solver():
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if self.cri_name == "BCE_loss": #如果是BCEloss 进行加权
                loss = self.criterion(output, target, weight_rate=self.weight).to(self.device)
            else:
    ...
    def validate_epoch(self, epoch):
        val_path = './validation'
        val_path = os.path.join(val_path,str(epoch))
        ...
        for batch_idx, (data, target) in enumerate(self.val_loader):
            ...
            if epoch%5==0:
                out = output[0].cpu().numpy().astype(np.uint8) * 255  # *255
                img = sitk.GetImageFromArray(out)
                sitk.WriteImage(img, os.path.join(val_path,str(batch_idx)+'_output.nii.gz'))

                out = data[0,0].cpu().numpy().astype(np.uint8)
                out = sitk.GetImageFromArray(out)
                sitk.WriteImage(out, os.path.join(val_path,str(batch_idx)+'_raw.nii.gz'))
                # import pdb
                # pdb.set_trace()
                out = target[0,0].cpu().numpy().astype(np.uint8)* 255
                out = sitk.GetImageFromArray(out)
                sitk.WriteImage(out, os.path.join(val_path,str(batch_idx)+'_target.nii.gz'))
